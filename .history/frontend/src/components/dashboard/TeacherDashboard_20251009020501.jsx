...
import TeacherBatchDetails from '../teacher/TeacherBatchDetails';
import { 
  BookOpen, 
  Users, 
  Calendar, 
...
import SubscriptionStatusBanner from '../teacher/SubscriptionStatusBanner';
...
import { useBatchCreationMessages } from '../../hooks/useUrlParams';
...
const TeacherDashboard = () => {
  ...
  // Handle URL parameters for tab navigation and batch creation messages
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const tabParam = urlParams.get('tab');
    if (tabParam && ['dashboard', 'batches', 'students', 'materials', 'attendance', 'payments', 'subscription', 'notifications', 'settings'].includes(tabParam)) {
    ...
    if (batchCreated === 'true') {
      toast({
        ...
        description: `${message || 'Your batch has been created successfully.'} ${batchName ? `Batch: ${batchName}` : ''} ${batchId ? `(ID: ${batchId})` : ''}`,
        variant: "default"
      });
