...
const BatchDetails = ({ batch, onBack, userRole, currentUser }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        try {
            ...
            toast({ title: "Error", description: "Failed to fetch some batch details.", variant: "destructive" });
        } finally {
            setIsLoading(false);
        }
    };
  ...
  const handleCreateNotification = async (e) => {
    ...
    try {
        await createNotification(batch.id, data);
        const notifications = await getNotificationsForBatch(batch.id);
        setBatchNotifications(notifications);
              ...
              <CardContent className="space-y-4">
                <form onSubmit={handleCreateNotification}>
                  ...
                  <div>
                    ...
                    <Textarea id="message" name="message" placeholder="Enter notification message" required />
                  </div>
