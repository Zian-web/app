...
import StudentDuePayments from './StudentDuePayments';
import StudentPaymentComponent from './StudentPaymentComponent';
...
const StudentBatchDetails = ({ batch, onBack, currentUser }) => {
  ...
  const fetchTeacherInfo = async () => {
    ...
    try {
      // Try to get teacher info from batch data or user data
      const response = await api.get(ENDPOINTS.USERS.ME);
      const userData = response.data || response;
      
      // Check if current user is a teacher (for teacher viewing their own batch)
      if (userData.type === 'teacher') {
        setTeacherInfo({ 
          requires_online_payment: userData.requires_online_payment || true,
          name: userData.full_name || userData.name
        });
      } else {
        // For students, assume teacher has online payment enabled
        // This should be updated when we have a proper teacher info endpoint
        setTeacherInfo({ requires_online_payment: true });
      }
    } catch (error) {
      console.error('Error fetching teacher info:', error);
      // Fallback for testing - assume online payment is available
      setTeacherInfo({ requires_online_payment: true });
    } finally {
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="payments">
          <div className="space-y-6">
            {/* Comprehensive Payment Component */}
            <StudentPaymentComponent 
              batchId={batch.id}
              onPaymentUpdate={fetchPayments}
            />
            
            {/* Payment History Section - Only Paid Payments */}
