...
const StudentDuePayments = ({ batchId, onPaymentUpdate }) => {
  ...
  const payForMonth = async (paymentId) => {
    try {
      ...
      const createResponse = await api.post(ENDPOINTS.PAYMENTS.STUDENT.CREATE_PAYMENT_LINK(batchId, 1));
  ...
  const payMultipleMonths = async (months) => {
    try {
      setProcessingPayment(true);
      const response = await api.post(ENDPOINTS.PAYMENTS.ONLINE.INITIATE_MULTIPLE, {
        batch_id: batchId,
        months: months,
        payment_mode: 'online'
      });
      ...
      toast({
        ...
        description: `Payment for ${months} months initiated`,
      });
      ...
      console.error('Error initiating multiple payments:', error);
      toast({
        title: "Payment Failed",
        description: error.response?.data?.detail || "Failed to initiate payment",
        variant: "destructive"
