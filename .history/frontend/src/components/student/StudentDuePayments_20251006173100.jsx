...
const StudentDuePayments = ({ batchId, onPaymentUpdate }) => {
  ...
  const payForMonth = async (paymentId, months = 1) => {
    try {
      ...
      console.log('Individual payment clicked - paymentId:', paymentId, 'months:', months, 'batchId:', batchId);
      ...
      console.log('Making API call for individual payment...');
      
      // Use direct redirect approach instead of trying to parse JSON
      const paymentUrl = `${API_BASE_URL}/api/payments/online/initiate`;
      const formData = new FormData();
      formData.append('batch_id', batchId);
      formData.append('months', months);
      formData.append('payment_mode', 'online');
      
      // Create a form and submit it to get the payment page
      ...
      // Add form data
      Object.entries({ batch_id: batchId, months: months, payment_mode: 'online' }).forEach(([key, value]) => {
  ...
  const payMultipleMonths = async (months) => {
    try {
      ...
      // Handle 6-month limit by chunking payments
      if (months > 6) {
        toast({
          title: "Payment Limit Exceeded",
          description: `You can only pay up to 6 months at a time. You have ${months} months due. Please pay in chunks of 6 months or less.`,
          variant: "destructive"
        });
        return;
      }
      ...
      } else if (error.response?.data?.detail?.includes("Months must be between 1 and 6")) {
        toast({
          title: "Payment Limit Exceeded",
          description: "You can only pay up to 6 months at a time. Please select 6 months or fewer.",
          variant: "destructive"
        });
      } else {
        toast({
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {/* Pay All Button with 6-month limit handling */}
            {duePayments.due_payments.length > 1 && (
              <div className="space-y-3">
                {duePayments.months_due > 6 ? (
                  <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-semibold text-yellow-800">Payment Limit Notice</h4>
                        <p className="text-sm text-yellow-600">
                          You can only pay up to 6 months at a time. You have {duePayments.months_due} months due.
                        </p>
                      </div>
                    </div>
                    <div className="mt-3 flex gap-2">
                      <Button 
                        onClick={() => payMultipleMonths(6)}
                      ...
                      <Button 
                        onClick={() => payMultipleMonths(Math.min(6, duePayments.months_due - 6))}
                        disabled={processingPayment}
