              ...
              {displayedPayments.map((payment) => (
                <div key={payment.id} className="bg-gray-50 rounded-lg p-4 border">
                  <div className="flex justify-between items-start mb-2">
                    {userRole === 'teacher' && <div className="font-medium text-sm">{getStudentName(payment.student_id || payment.studentId)}</div>}
                ...
                <TableBody>
                  {displayedPayments.map((payment) => (
                    <TableRow key={payment.id}>
                      {userRole === 'teacher' && <TableCell className="font-medium w-[20%]">{getStudentName(payment.student_id || payment.studentId)}</TableCell>}
