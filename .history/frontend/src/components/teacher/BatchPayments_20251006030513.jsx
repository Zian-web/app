                ...
                <TableBody>
                  {displayedPayments.map((payment) => (
                    <TableRow key={payment.id}>
                      {userRole === 'teacher' && <TableCell className="font-medium w-[20%]">{payment.student_name || getStudentName(payment.student_id || payment.studentId)}</TableCell>}
                      <TableCell className="w-[15%]">₹{payment.amount}</TableCell>
                      <TableCell className="w-[20%]">{payment.due_date || payment.dueDate}</TableCell>
