          ...
          <div className="hidden md:block">
            ...
            >
              <Table className="w-full">
                <TableHeader>
                  <TableRow>
                    {userRole === 'teacher' && <TableHead className="w-[15%]">Student</TableHead>}
                    <TableHead className="w-[10%]">Amount</TableHead>
                    <TableHead className="w-[10%]">Months</TableHead>
                ...
                <TableBody>
                  {displayedPayments.map((payment) => (
                    <TableRow key={payment.id}>
                      {userRole === 'teacher' && <TableCell className="font-medium w-[15%]">{payment.student_name || getStudentName(payment.student_id || payment.studentId)}</TableCell>}
                      <TableCell className="w-[10%]">₹{payment.amount}</TableCell>
                      <TableCell className="w-[10%]">{payment.months_paid || payment.monthsPaid || (payment.amount / (payment.batch_fees || 500)) || 1}</TableCell>
                      <TableCell className="w-[15%]">{payment.due_date || payment.dueDate}</TableCell>
                      <TableCell className="w-[10%]">
                        <Badge variant={payment.payment_status === 'paid' || payment.status === 'paid' ? 'success' : payment.payment_status === 'pending' || payment.status === 'pending' ? 'warning' : 'destructive'}>
                          {payment.payment_status || payment.status}
                        </Badge>
                      </TableCell>
                      <TableCell className="w-[15%]">{payment.paid_date || payment.paidDate || 'N/A'}</TableCell>
                      <TableCell className="w-[10%]">{payment.payment_mode || payment.paymentMode || 'Online'}</TableCell>
