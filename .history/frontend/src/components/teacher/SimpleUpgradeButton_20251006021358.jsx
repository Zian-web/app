...
const SimpleUpgradeButton = () => {
  ...
  const checkPaymentStatus = async () => {
    try {
      // Try to get teacher data from /api/users/me first
      ...
      // Check if payment fields exist in the response
      const hasPaymentFields = teacherData && (
        'requires_online_payment' in teacherData ||
        'has_razorpay_account' in teacherData ||
        'razorpay_account_id' in teacherData ||
        'razorpay_account_status' in teacherData
      );
      
      if (hasPaymentFields) {
        // Payment fields exist, check if online payment is enabled
        ...
        // No payment fields in /api/users/me, try to get teacher-specific data
        console.log('No payment fields found in /api/users/me, trying teacher endpoint...');
        try {
          // Try different possible teacher endpoints
          let teacherResponse = null;
          try {
            teacherResponse = await api.get('/api/teacher/profile');
            console.log('Teacher profile endpoint response:', teacherResponse);
          } catch (profileError) {
            console.log('Teacher profile endpoint failed, trying teacher info...');
            try {
              teacherResponse = await api.get('/api/teacher/info');
              console.log('Teacher info endpoint response:', teacherResponse);
            } catch (infoError) {
              console.log('Teacher info endpoint failed, trying teacher details...');
              try {
                teacherResponse = await api.get('/api/teacher/details');
                console.log('Teacher details endpoint response:', teacherResponse);
              } catch (detailsError) {
                console.log('All teacher endpoints failed, checking if teacher has Razorpay account...');
                // If no teacher endpoint works, assume teacher doesn't have online payment
                setIsEnabled(false);
                return;
              }
            }
          ...
          if (teacherResponse.data) {
            const teacherPaymentData = teacherResponse.data;
            const hasOnlinePayment = teacherPaymentData && (
              teacherPaymentData.requires_online_payment === true || 
              teacherPaymentData.requires_online_payment === 'true' ||
              teacherPaymentData.has_razorpay_account === true ||
              teacherPaymentData.has_razorpay_account === 'true' ||
              teacherPaymentData.razorpay_account_id ||
              teacherPaymentData.razorpay_account_status === 'active'
            );
            
            if (hasOnlinePayment) {
              console.log('Teacher has online payment enabled (from teacher endpoint)');
              setIsEnabled(true);
            } else {
              console.log('Teacher does not have online payment enabled (from teacher endpoint)');
              setIsEnabled(false);
            }
          } else {
            console.log('No teacher payment data available');
            setIsEnabled(false);
          }
        } catch (teacherError) {
          console.error('Error getting teacher data:', teacherError);
          console.log('Assuming teacher does not have online payment enabled');
          setIsEnabled(false);
        }
      }
    } catch (error) {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      // Also try to get fresh teacher data to see what changed
      try {
        const freshResponse = await api.get('/api/users/me');
        console.log('Fresh teacher data after Razorpay creation:', freshResponse);
        console.log('Fresh teacher data fields:', Object.keys(freshResponse.data || freshResponse || {}));
      } catch (freshError) {
        console.error('Error getting fresh teacher data:', freshError);
      }
