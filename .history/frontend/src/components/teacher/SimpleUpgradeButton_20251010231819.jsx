...
const SimpleUpgradeButton = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phoneNumber: '+91',
    panNumber: '', // NEW: PAN number for KYC
    street: '',
    city: '',
    state: '',
    postalCode: ''
  ...
  const checkPaymentStatus = async () => {
    try {
      ...
      if (hasPaymentFields) {
        ...
        // No payment fields in /api/users/me, check if teacher has Razorpay account
        console.log('No payment fields found in /api/users/me, checking Razorpay account status...');
        try {
          // Try to check if teacher has a Razorpay account by calling the Razorpay account endpoint
          const razorpayResponse = await api.get('/api/teacher/razorpay-account');
          console.log('Razorpay account check response:', razorpayResponse);
          
          if (razorpayResponse.data && razorpayResponse.data.id) {
            console.log('Teacher has Razorpay account:', razorpayResponse.data);
            setIsEnabled(true);
          } else {
            console.log('Teacher does not have Razorpay account');
            setIsEnabled(false);
          }
        } catch (razorpayError) {
          console.log('Razorpay account check failed (teacher likely doesn\'t have account):', razorpayError.message);
          setIsEnabled(false);
        }
      }
    } catch (error) {
