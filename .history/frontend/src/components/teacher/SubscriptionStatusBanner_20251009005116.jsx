...
const SubscriptionStatusBanner = () => {
  ...
  // If subscription payment is required, show warning
  if (!paymentStatus.subscription_paid && paymentStatus.has_subscription) {
    return (
      <Alert className="border-orange-500 bg-orange-50 mb-4">
        ...
        <AlertDescription>
          <div className="flex items-center justify-between">
            <div>
              <strong className="text-orange-800">Subscription Payment Required</strong>
              <p className="text-sm text-orange-700 mt-1">
                {paymentStatus.message || `You have ${paymentStatus.unpaid_count} unpaid subscription(s). Please pay for your subscription.`}
              </p>
              <p className="text-xs text-orange-600 mt-1">
                Check the "Batch Subscriptions" section below to see individual batch payments.
              </p>
            </div>
            <div className="flex gap-2">
              <Badge variant="destructive">
  ...
  // If monthly payment is overdue but within grace period, show warning
  if (monthlyPaymentStatus.monthly_payment_overdue && !monthlyPaymentStatus.batches_locked) {
    return (
      <Alert className="border-yellow-500 bg-yellow-50 mb-4">
        ...
        <AlertDescription>
          <div className="flex items-center justify-between">
            <div>
              <strong className="text-yellow-800">Monthly Payment Overdue</strong>
              <p className="text-sm text-yellow-700 mt-1">
                {monthlyPaymentStatus.message || 'Monthly payment overdue. Please pay soon to avoid batch locking.'}
              </p>
            </div>
            <div className="flex gap-2">
