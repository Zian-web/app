...
const fetchMaterials = async () => {
  try {
    // 1. Get teacher batches
    const batchesRes = await axios.get("/api/batches/");
    const batches = batchesRes.data;

    // 2. Fetch materials for each batch
    const materialsPromises = batches.map((batch) =>
      axios.get(`/api/batches/${batch.id}/materials/`).then(res => 
        res.data.map(mat => ({ ...mat, batch }))
      )
    );
