...
const TeacherMaterials = ({ userRole }) => {
  ...
  // Fetch grouped materials from new API endpoint
  const fetchMaterials = async () => {
    try {
      const res = await axios.get("/api/teacher/materials/grouped");
      setGroupedMaterials(res.data); // API already returns grouped data
    } catch (err) {
  ...
  // Filter grouped materials based on search
  const filteredGroupedMaterials = Object.keys(groupedMaterials).reduce(
    (acc, batchName) => {
      const materialsArray = groupedMaterials[batchName]?.materials || [];
      const filtered = materialsArray.filter(
        (material) =>
