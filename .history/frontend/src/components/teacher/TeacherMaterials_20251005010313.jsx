...
// UPDATED: Import the correct function for grouped materials
import { getGroupedMaterials, getMyBatches, deleteMaterial } from "../../lib/api";
import { Input } from "../ui/input";

const TeacherMaterials = () => {
    // UPDATED: State now holds grouped materials directly
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            // Fetch both pre-grouped materials and batches in parallel
            const [materialsData, batchesData] = await Promise.all([
                getGroupedMaterials(), // Use the correct API function
                getMyBatches(),
            ]);
            setGroupedMaterials(materialsData || []);
    ...
    // Filter the already grouped materials based on search query
