...
import {
    ...
    deleteMaterial,
    uploadMaterial,
    getBatchMaterials, // <-- Import getBatchMaterials
    api
} from "../../lib/api";
...
const TeacherMaterials = () => {
  ...
  const { checkMaterialUploadPermission, subscriptionStatus } = useSubscriptionPermissions();
  ...
  // UPDATED: New logic to get presigned URL
  const handleViewMaterial = async (material, batchId) => {
    try {
        const materialsForBatch = await getBatchMaterials(batchId);
        const targetMaterial = materialsForBatch.find(m => m.id === material.id);

        if (targetMaterial && targetMaterial.access_url) {
            setViewingPdf(targetMaterial);
        } else {
            throw new Error("Presigned URL not found for this material.");
        }
    } catch (error) {
        console.error("Error getting material for viewing", error);
        toast({
            title: "Error",
  ...
  const handleUploadMaterial = async (e) => {
    ...
    // Check subscription permission for each selected batch
    for (const batchId of selectedBatchIds) {
      const canUpload = await checkMaterialUploadPermission(batchId);
      if (!canUpload) {
        toast({
          title: "Subscription Required",
          description: `You need an active subscription to upload materials to batch ${batchId}. Please subscribe first.`,
          variant: "destructive"
        });
        return;
      }
    ...
    try {
      ...
      setUploadFile(null);
      setSelectedBatchIds([]);
      document.getElementById('material-upload-file-input').value = '';
      fetchGroupedMaterials();
    } catch (err) {
      ...
      if (err.message && err.message.includes('Subscription required')) {
        toast({
          title: "Subscription Required",
          description: "You need an active subscription to upload materials. Please subscribe first.",
          variant: "destructive"
        });
      } else if (err.message && err.message.includes('Materials locked')) {
        toast({
                ...
                <CardContent className="p-4 flex justify-between items-center">
                  ...
                  <div className="flex items-center space-x-2">
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => handleViewMaterial(material, group.batch_id)} // Pass batch_id
                    >
