...
const TeacherSubscriptionManager = ({ teacherId }) => {
  ...
  const fetchSubscriptionData = async () => {
    ...
    try {
      ...
      const statusResponse = await fetch(`${API_BASE_URL}${ENDPOINTS.SUBSCRIPTION.TEACHER.STATUS}`, {
      ...
      const metricsResponse = await fetch(`${API_BASE_URL}${ENDPOINTS.SUBSCRIPTION.TEACHER.METRICS}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const subscriptionsResponse = await fetch(`${API_BASE_URL}${ENDPOINTS.SUBSCRIPTION.TEACHER.SUBSCRIPTIONS}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const createSubscription = async (plan = 'basic') => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${ENDPOINTS.SUBSCRIPTION.TEACHER.CREATE}?plan=${plan}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      if (response.ok) {
        ...
        toast({
          title: "Error",
          description: error.detail || "Failed to create subscription",
          variant: "destructive"
        });
      }
    } catch (error) {
