...
const TeacherSubscription = () => {
  ...
  const fetchSubscriptionData = async () => {
    try {
      ...
      const statusResponse = await api.get('/api/teacher/subscription/status-check');
      setSubscriptionStatus(statusResponse.data);
      
      // Fetch subscription metrics
      const metricsResponse = await api.get('/api/teacher/subscription/metrics');
      setSubscriptionMetrics(metricsResponse.data);
      
      // Fetch subscription payments
      const paymentsResponse = await api.get('/api/teacher/subscription/payments');
      setSubscriptionPayments(paymentsResponse.data);
  ...
  const createSubscription = async (plan = 'basic') => {
    try {
      setIsCreating(true);
      await api.post('/api/teacher/subscription/create', null, {
        params: { plan }
      });
  ...
  const processSubscriptionPayment = async (subscriptionId) => {
    try {
      await api.post('/api/teacher/subscription/payment', null, {
        ...
        <Alert className={subscriptionStatus.materials_locked ? "border-red-500 bg-red-50" : "border-green-500 bg-green-50"}>
          ...
          <AlertDescription>
            {subscriptionStatus.materials_locked ? (
              <div className="flex items-center justify-between">
                <div>
                  <strong>Materials Locked:</strong> {subscriptionStatus.lock_reason}
      ...
      {subscriptionMetrics && (
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{subscriptionMetrics.student_count || 0}</p>
                <p className="text-sm text-gray-600">Total Students</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-green-600">{subscriptionMetrics.batch_count || 0}</p>
