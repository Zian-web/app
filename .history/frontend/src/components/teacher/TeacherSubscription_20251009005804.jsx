...
const TeacherSubscription = () => {
  ...
  const processSubscriptionPayment = async (subscriptionId, months = 1) => {
    try {
      const response = await api.post(`${ENDPOINTS.SUBSCRIPTION.TEACHER.PAYMENT}?subscription_id=${subscriptionId}&months=${months}`);
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <Badge variant={subscription.is_due || subscription.payment_status === 'pending' || (!subscription.payment_status && subscription.monthly_fee > 0) ? 'destructive' : subscription.payment_status === 'paid' ? 'default' : 'secondary'}>
                          {subscription.is_due || subscription.payment_status === 'pending' || (!subscription.payment_status && subscription.monthly_fee > 0) ? 'Payment Due' : subscription.payment_status === 'paid' ? 'Paid' : 'Pending'}
                        </Badge>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                        ...
                        <div>
                          ...
                          <p className="font-medium text-lg">₹{subscription.monthly_fee}</p>
                        </div>
                        <div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      {subscription.is_due || subscription.payment_status === 'pending' || (!subscription.payment_status && subscription.monthly_fee > 0) ? (
                        <Button 
                          onClick={() => processSubscriptionPayment(subscription.subscription_id)}
                          className="flex items-center gap-2"
                        >
                          <CreditCard className="h-4 w-4" />
                          Pay Now (₹{subscription.monthly_fee || 'Calculate'})
                        </Button>
