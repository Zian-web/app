...
const TeacherSubscription = () => {
  ...
  const processSubscriptionPayment = async (subscriptionId, months = 1) => {
    try {
      ...
      const paymentUrl = response?.payment_link_url || 
      ...
      if (paymentUrl) {
        ...
        // Check if the response indicates success without redirect
        if (response?.success || response?.status === 'success' || response?.message?.includes('success')) {
          ...
          toast({
            title: "Error",
            description: "No payment URL received from server. Response: " + JSON.stringify(response),
            variant: "destructive"
          });
        }
      }
