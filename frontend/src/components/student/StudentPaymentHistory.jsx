...
const StudentPaymentHistory = ({ payments, userRole, getStudentName, getPaymentStatusColor, onUpdatePayment, batchId, teacherHasOnlinePayment }) => {
  ...
  // Filter to only show PAID payments - simplified without pagination
  ...
  // Update displayed payments when paidPayments changes
  useEffect(() => {
    setDisplayedPayments(paidPayments);
  }, [paidPayments]);

  // Removed pagination logic to prevent infinite loops
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {/* Removed pagination to prevent infinite loops */}
        </div>
      </CardContent>
    </Card>
  );
